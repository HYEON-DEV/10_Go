...
func GenerateSignHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 업로드된 파일 읽기
	file, _, err := r.FormFile("privateKeyFile")
	if err != nil {
...
func main() {
	...
	defer logFile.Close() // ✅ defer 은 뭐야야
	...
	router.HandleFunc("/api/generate_key", GenerateKeyHandler).Methods("POST")
	router.HandleFunc("/api/generate_sign", GenerateSignHandler).Methods("POST")
